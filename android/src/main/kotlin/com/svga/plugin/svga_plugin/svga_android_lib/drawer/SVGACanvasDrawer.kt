...
import com.svga.plugin.svga_plugin.svga_android_lib.entities.SVGAVideoShapeEntity
    ...
    private fun drawShape(sprite: SVGADrawerSprite, canvas: Canvas) {
        ...
        sprite.frameEntity.shapes.forEach { shape ->
            ...
            shape.shapePath?.let {
                ...
                shape.styles?.strokeWidth?.let { it ->
                    if (it > 0) {
                        paint.alpha = (sprite.frameEntity.alpha * 255).toInt()
                        paint.style = Paint.Style.STROKE
                        shape.styles?.stroke?.let {
                            paint.color = it
                            val alpha = 255.coerceAtMost(0.coerceAtLeast((sprite.frameEntity.alpha * 255).toInt()))
                            if (alpha != 255) {
                                paint.alpha = alpha
                            }
                        }
